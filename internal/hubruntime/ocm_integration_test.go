...
func TestHubRuntimeDirectClusterProxyM0(t *testing.T) {
	...
	handler, err := hubserver.NewFleetHandler(hubserver.FleetHandlerConfig{
		Verifier: verifier, Collector: collector, Reader: store, ImageSearcher: imageSearcher, CVESearcher: cveSearcher, PEP: enforcer,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func (store *m0RuntimeStore) QueryFleet(
	...
	if len(query.Kinds) != 1 || fleet.ValidateImageDigest(query.Selector.Image) != nil || freshness < time.Second || now.IsZero() {
		return fleet.QueryResult{}, fmt.Errorf("M0 runtime store received an unsupported fleet query")
	}
	wantInventory := query.Kinds[0] == fleet.FactInventory && query.Selector.ResourceKind == "Pod"
	wantCVE := query.Kinds[0] == fleet.FactCVE && query.Selector.ResourceKind == "Image"
	if !wantInventory && !wantCVE {
		return fleet.QueryResult{}, fmt.Errorf("M0 runtime store received an unsupported fleet query")
	}
	store.mu.Lock()
	defer store.mu.Unlock()
	...
	for _, spoke := range store.spokes {
		...
		for _, evidence := range snapshot.Facts {
			...
			if wantCVE && (evidence.Kind != fleet.FactCVE || evidence.Ref.Kind != "Image" || !m0RuntimeCVEFactHasDigest(evidence, query.Selector.Image)) {
				continue
			}
