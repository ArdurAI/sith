...
func TestPortableAlertRulesStayBoundedAndStatic(t *testing.T) {
	...
	want := map[string]struct {
		...
		"SithHubAuthenticationRefusalOnly": {
			severity: "warning", hold: "10m",
			expr:        `sum(increase(sith_auth_attempts_total{outcome="refused"}[15m])) >= 20 and sum(increase(sith_auth_attempts_total{outcome="accepted"}[15m])) == 0`,
			summary:     "Sith hub authentication traffic is persistently refusal-only",
			description: "At least twenty aggregate authentication attempts were refused and none were accepted over the last fifteen minutes.",
		},
